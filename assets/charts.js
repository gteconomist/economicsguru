...
window.EG = {
  ...
  renderInflation(data) {
    ...
    new Chart(document.getElementById('chartMom'), {
      ...
      options: baseOptions(v => `${v >= 0 ? '+' : ''}${v.toFixed(2)}%`, { scales: { beginAtZero: true } })
    });
    ...
    new Chart(document.getElementById('chartComp'), {
      ...
      data: {
        labels,
        datasets: [
          { label: 'Food',     data: data.food_yoy.map(r => r[1]),     borderColor: BRAND.green,   backgroundColor: BRAND.green,   tension: 0.2, borderWidth: 2.2, pointRadius: 2.5 },
          { label: 'Energy',   data: data.energy_yoy.map(r => r[1]),   borderColor: BRAND.coral,   backgroundColor: BRAND.coral,   tension: 0.2, borderWidth: 2.2, pointRadius: 2.5 },
          { label: 'Shelter',  data: data.shelter_yoy.map(r => r[1]),  borderColor: BRAND.teal,    backgroundColor: BRAND.teal,    tension: 0.2, borderWidth: 2.2, pointRadius: 2.5 },
          { label: 'Services', data: data.services_yoy.map(r => r[1]), borderColor: BRAND.mustard, backgroundColor: BRAND.mustard, tension: 0.2, borderWidth: 2.2, pointRadius: 2.5 }
        ]
      },
    ...
    new Chart(document.getElementById('chartEnergy'), {
      ...
      data: {
        ...
        datasets: [
          { label: 'Gasoline (start = 100)', data: data.gasoline_idx.map(r => r[1]),
            borderColor: BRAND.coral, backgroundColor: BRAND.coral, tension: 0.2, borderWidth: 2.5, pointRadius: 3 },
          { label: 'Energy, all (start = 100)', data: data.energy_idx.map(r => r[1]),
            borderColor: BRAND.mustard, backgroundColor: BRAND.mustard, tension: 0.2, borderWidth: 2.5, pointRadius: 3 }
        ]
      },
      options: baseOptions(v => v.toFixed(1))
    });

    // Register CSV downloads after charts are built
    registerCsv('chartYoy',
      'headline-vs-core-cpi.csv',
      ['Month', 'Headline CPI YoY (%)', 'Core CPI YoY (%)'],
      mergeSeries([data.headline_yoy, data.core_yoy]));

    registerCsv('chartMom',
      'cpi-monthly-change-sa.csv',
      ['Month', 'Headline CPI MoM SA (%)', 'Core CPI MoM SA (%)'],
      mergeSeries([data.headline_mom_sa, data.core_mom_sa]));

    registerCsv('chartComp',
      'cpi-components-yoy.csv',
      ['Month', 'Food YoY (%)', 'Energy YoY (%)', 'Shelter YoY (%)', 'Services YoY (%)'],
      mergeSeries([data.food_yoy, data.energy_yoy, data.shelter_yoy, data.services_yoy]));

    registerCsv('chartEnergy',
      'energy-prices-indexed.csv',
      ['Month', 'Gasoline (Index)', 'Energy (Index)'],
      mergeSeries([data.gasoline_idx, data.energy_idx]));
